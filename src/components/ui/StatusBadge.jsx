import { CheckCircle, Clock, AlertTriangle, Loader2, FileText, AlertCircle } from 'lucide-react';
...
const StatusBadge = ({ status }) => {
    ...
    const config = {
        ...
        processing: { color: "text-blue-400 border-blue-400 shadow-blue-400/50", icon: Loader2, animate: true },
        transcribing: { color: "text-blue-400 border-blue-400 shadow-blue-400/50", icon: Loader2, animate: true },
        extracting: { color: "text-neon-purple border-neon-purple shadow-neon-purple/50", icon: Loader2, animate: true },
    ...
    const Icon = current.icon || AlertCircle; // Fallback to AlertCircle if icon is missing
    ...
    return (
        <span className={`flex items-center gap-2 px-3 py-1 text-xs font-display uppercase border rounded-full shadow-[0_0_5px_rgba(0,0,0,0.2)] ${current.color} bg-bg-dark/50 backdrop-blur-sm`}>
            {Icon && <Icon size={14} className={current.animate ? "animate-spin" : ""} />}
            {s.replace(/_/g, ' ')}
        </span>
    );
};
